...
from src.datasets.dataset import CelebAHQDataset, get_transforms, TO_TENSOR, NORMALIZE, MASK_CONVERT_TF, CustomDataset, FFHQDataset, FFHQ_MASK_CONVERT_TF, MASK_CONVERT_TF_DETAILED, FFHQ_MASK_CONVERT_TF_DETAILED
...
class Coach:
    ...
    def __init__(self, opts):
        ...
        if self.opts.checkpoint_path is not None:
            ...
            if self.opts.dist_train:    
                ...
                #changed code start
                    state_dict = ckpt_dict["D_state_dict"]
                    from collections import OrderedDict
                    new_state_dict = OrderedDict()
                    for k, v in state_dict.items():
                        name = k[7:]
                        new_state_dict[name] = v
                    self.D.module.load_state_dict(new_state_dict) 
                #end
            else:
    ...
    def configure_datasets(self):
        train_ds = CustomDataset(dataset_root=self.opts.ffhq_dataset_root,
        ...
        test_ds = CustomDataset(dataset_root=self.opts.celeba_dataset_root, mode="test",
                                  img_transform=transforms.Compose(
                                      [TO_TENSOR, NORMALIZE]),
                                  label_transform=transforms.Compose(
    ...
    def train(self):
        ...
        while self.global_step <= self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):    
                ...
                img, mask, img_random, mask_random = batch 
                flag = (random.random()>r)
                # if global_step%1000 == 0:
                #     r = r*0.99
                    
                if flag:
                    mask_init = mask #source mask torch.where(mask > 0, 1)
                    mask = mask_random #target mask torch.where(mask > 0, 1)

                img = img.to(self.device).float()  #src_image
                img_random = img_random.to(self.device).float()
                mask = (mask*255).long().to(self.device)
                bin_mask = torch.where(mask==0,0,1).to(self.device).float()
                onehot0 = bin_mask #(mask/255).float() #torch_utils.labelMap2OneHot(mask, num_cls=self.opts.num_seg_cls)
                ...
                if self.opts.train_D and (self.global_step % self.opts.d_every == 0):
                    ...
                    recon1, _, latent = self.net(img, onehot0, return_latents=True)
                    # print('recon1 size:',recon1.size())
                    # break
                    fake_pred_1 = self.D(recon1*bin_mask + img_random*(1-bin_mask))
                    ...
                    if flag:
                        d_loss_dict["d_loss_cross"]=float(d_loss)
                        d_loss_dict["real_score_cross"]=float(real_pred.mean())
                        d_loss_dict["fake_score_1_cross"]=float(fake_pred_1.mean())
                    else:
                        d_loss_dict["d_loss_pairwise"]=float(d_loss)
                        d_loss_dict["real_score_pairwise"]=float(real_pred.mean())
                        d_loss_dict["fake_score_1_pairwise"]=float(fake_pred_1.mean())
                ...
                if flag:
                    print('cycle ', flag)
                    recon0, _, latent = self.net(img, onehot0, return_latents=True)
                    g_loss = torch.tensor(0.0, device=self.device)            
                        
                    if self.opts.train_D:
                        fake_pred_1 = self.D(recon0*bin_mask + img_random*(1-bin_mask))
                        g_loss = self.adv_g_loss(fake_pred_1) #-------------------
                    
                    
                    mask = mask_init
                    mask = (mask*255).long().to(self.device)
                    mask = torch.where(mask==0,0,1).to(self.device).float()
                    
                    loss_, loss_dict, id_logs = self.calc_loss(img*mask, recon0*bin_mask, bin_mask, latent, flag=flag) #------------
                    loss_dict["g_loss_cross"] = float(g_loss)
                    overall_loss = loss_ + self.opts.g_adv_lambda * g_loss
                    loss_dict["loss_cross"] = float(overall_loss)
                    ...
                    recon0 = recon0.detach()
                    recon0 = bin_mask*recon0 + (1-bin_mask)*img_random

                    mask = mask_init
                    mask = (mask*255).long().to(self.device)
                    bin_mask = torch.where(mask==0,0,1).to(self.device).float()
                    onehot = bin_mask #(mask/255).float() #torch_utils.labelMap2OneHot(mask, num_cls=self.opts.num_seg_cls)
                    recon1, _, latent = self.net(recon0, onehot, return_latents=True)#----------
                    
                    g_loss = torch.tensor(0.0, device=self.device)      
                             
                    if self.opts.train_D:
                        fake_pred_1 = self.D(recon1*bin_mask + img*(1-bin_mask))
                        g_loss = self.adv_g_loss(fake_pred_1)#-----------------
                    
                    loss_, loss_dict, id_logs = self.calc_loss(img*bin_mask, recon1*bin_mask, bin_mask, latent, flag=not flag)
                    
                    overall_loss = loss_ + self.opts.g_adv_lambda * g_loss
                    self.net.zero_grad()
                    overall_loss.backward()
                    self.optimizer.step()
                    recon1 = recon1*bin_mask + (1-bin_mask)*img
                    ...
                    loss_, loss_dict, id_logs = self.calc_loss(img*bin_mask, recon1*bin_mask, bin_mask, latent, flag=flag)
                    overall_loss = loss_ + self.opts.g_adv_lambda * g_loss
                ...
                if self.rank==0 and (self.global_step % self.opts.image_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0)):
                    if flag:
                        imgs = self.parse_images(onehot0, onehot, img, recon0, recon1, img_random, flag = flag)
                    else:
                        imgs = self.parse_images(mask0=onehot0, img=img, recon1=recon1, flag = flag)
    ...
    def calc_loss(self, img, recon1, mask, latent, flag=False):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        ...
        if self.opts.id_lambda > 0:
            ...
            if flag:       
                loss_dict['loss_id_corss'] = float(loss_id_1)
                loss_dict['id_improve_cross'] = float(sim_improvement_1)

            else:
                loss_dict['loss_id_pairwise'] = float(loss_id_1)
                loss_dict['id_improve_pairwise'] = float(sim_improvement_1)
                
            loss += loss_id_1 * self.opts.id_lambda

        if self.opts.l2_lambda > 0 and not(flag):
            
            loss_l2_1 = F.mse_loss(recon1, img)
            loss_dict['loss_l2'] = float(loss_l2_1)
            loss += loss_l2_1 * self.opts.l2_lambda
            
        if self.opts.lpips_lambda > 0 and not(flag):
            loss_lpips = 0
            for i in range(3):
    ...
    def parse_images(self, mask0=None, mask1=None, img=None, recon0=None, recon1=None, random_face=None, display_count=2, flag = 0):
        im_data = []
        ...
        if flag:
            for i in range(display_count):
                cur_im_data = {
                    'source_face': torch_utils.tensor2im(img[i]),
                    'target_mask': torch_utils.tensor2map(mask0[i]),
                    'recon_t': torch_utils.tensor2im(recon0[i]),
                    'target_face': torch_utils.tensor2im(random_face[i]),
                    'source_mask': torch_utils.tensor2map(mask1[i]),
                    'recon_s': torch_utils.tensor2im(recon1[i]),
                }
            ...
            for i in range(display_count):
                cur_im_data = {
                    ...
                    'input_mask': torch_utils.tensor2map(mask0[i]),
                    'recon_styleCode': torch_utils.tensor2im(recon1[i]),
                    'input_random': torch_utils.tensor2im(img[i]),
                }
                im_data.append(cur_im_data)
    ...
    def log_images(self, name, imgs1_data, subscript=None, log_latest=False):
        # print('inside log images ------------_#####################\n')
        # print(imgs1_data)
        fig = torch_utils.vis_faces(imgs1_data, grid_size=6)
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            img, mask, _, _ = batch #, mask_vis
            
            with torch.no_grad():
                img = img.to(self.device).float()
                mask = (mask*255).long().to(self.device)
                # [bs,1,H,W] format mask to one-hot, i.e., [bs,#seg_cls,H,W]
                onehot = (mask/255).float() #torch_utils.labelMap2OneHot(mask, num_cls=self.opts.num_seg_cls)
            ...
            # if show_images:
                # imgs = self.parse_images(onehot, img, recon1, flag=0)
        ...
        return loss_dict
"""
python  -m torch.distributed.launch --nproc_per_node=1 --nnodes=1 --node_rank=0 --master_addr=localhost --master_port=22222 scripts/train.py --checkpoint_path=/home/gcpadmin/workplace_eth_change/running_exp_cycle/checkpoints/iteration_90000.pt
"""